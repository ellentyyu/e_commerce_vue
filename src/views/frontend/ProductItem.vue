      ...
      <section class="container-fluid container-custom pt-9 mb-8">
        <div class="row ind-pro">
          <div class="col-md-6">
            <div class="row g-3">
              <div class="col-12">
                <div class="photo-lg bg-cover" :style="`background-image:url(${getImgUrl(product.images[imgKey])});`" v-if="product.images"></div>
              </div>
              <template v-if="product.images">
                <div class="col-4 photo-sm-wrap" v-for="(image, index) in product.images" :key="index" @mouseover="imgKey=index">
                  <!-- 靜態圖片 url '/images/products/${image}' -->
                  <div class="photo-sm bg-cover position-relative" :style="`background-image:url(${getImgUrl(image)});`"> 
          ...
          <div class="col-md-6">
            <div class="mx-2 ms-md-3">
              <div class="d-flex align-items-center text-fade fs-s my-4 mt-md-0">
                <router-link to="/" class="ind-pro-nav">首頁</router-link>
                <span class="mx-2">/</span>
                <router-link to="/products" class="ind-pro-nav">商店</router-link>
                <span class="mx-2">/</span>
              ...
              <div class="d-md-flex col-md-8">
                ...
                <button class="btn-ind-pro col-12 col-md-7 mt-2 mt-md-0 position-relative" :class="{ 'disabled':product.id===status.loadingItem }" type="button" @click="addCart(product.id, qty)">加入購物車
