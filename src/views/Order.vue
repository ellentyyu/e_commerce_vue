            ...
            <ul>
              <li v-for="item in cart.carts" :key="item.id" class="mb-4 row g-3 align-items-center">
                <div class="col-auto d-none d-ss-block">
                  <div class="order-img bg-cover" :style="`background-image:url(${item.product.images[0]});`"></div>
                </div>
                <div class="col">
          ...
          <div class="customer px-2 pe-me-3 pe-lg-5">
            ...
            <Form @submit="createOrder" v-slot="{ errors }">
              <div class="form-item">
                ...
                <Field type="email" name="email" rules="email|required" id="email" class="form-control" :class="{'is-invalid':errors['email']}" placeholder="請輸入 Email" v-model="form.user.email"></Field>
                <ErrorMessage name="email" class="invalid-feedback mt-1"></ErrorMessage>
              </div>
              <div class="form-item">
                <label for="name" class="form-label">收件人姓名</label>
                <Field type="text" name="收件人姓名" rules="required" id="name" class="form-control" :class="{'is-invalid':errors['收件人姓名']}" placeholder="請輸入姓名" v-model="form.user.name"></Field>
                <ErrorMessage name="收件人姓名" class="invalid-feedback mt-1"></ErrorMessage>
              </div>
              <div class="form-item">
                <label for="phonenb" class="form-label">聯絡電話</label>
                <Field type="text" name="手機號碼" :rules="isPhone" id="phonenb" class="form-control" :class="{'is-invalid':errors['手機號碼']}" placeholder="請輸入手機號碼" v-model="form.user.tel"></Field>
                <ErrorMessage name="手機號碼" class="invalid-feedback mt-1"></ErrorMessage>
              </div>
              <div class="form-item">
                <label for="address" class="form-label">收件地址</label>
                <Field type="text" name="收件地址" rules="required" id="address" class="form-control" :class="{'is-invalid':errors['收件地址']}" placeholder="請輸入地址" v-model="form.user.address"></Field>
                <ErrorMessage name="收件地址" class="invalid-feedback mt-1"></ErrorMessage>
              </div>
              <div class="form-item">
