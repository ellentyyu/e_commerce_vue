  ...
  <Navbar>
    <div class="admin-coupons admin-table pt-4 pb-5" style="margin-left:-12px; margin-right:-12px">
      ...
      <div class="d-flex flex-column h-100">
        ...
        <Pagination class="mt-4" :pages="pagination" @emit-pages="getCoupons"/>
      </div>
      <CouponModal ref="couponModal" :coupon="tempCoupon" @update-coupon="updateCoupon"/>
      <DelModal ref="delModal" :delItem="delCoupon" @delete-item="deleteCoupon"/>
    </div>
  </Navbar>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    updateCoupon(item) {
      ...
      // 這裡若用tempCoupon可以運作，但會將timestamp傳回去modal裡，modal的date input會跳格式不符警告
